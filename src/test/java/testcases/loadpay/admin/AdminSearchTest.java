...
public class AdminSearchTest extends TestBase {
	...
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - All
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - Unmatched
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - Not Scheduled
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - Scheduled
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - Paid
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - Error
	// Verify filters - Search for payments with these statuses and verify they come
	// up correctly - Cancelled
	@Test(description = "LP-5423 Admin_Search_goToSearchPage", dependsOnMethods = {
			"goToSearchPage" }, dataProvider = "getAdminSearchData")
	public void searchTest(String paymentId, String invoiceAmountFrom, String invoiceAmountTo, String dateFrom,
		...
		// reset fields first so that the fields are reset on each iteration as
		// determined by the dataProvider
		adminSearchPage.resetFields();

		// check data provider elements for null values...if not null, setup the
		// relevant data in the web page
		if (paymentId != null) {
		...
		if (filterByAll != null) {
			if (filterByAll.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isAllFilterChecked(),
						"All filter NOT checked but data indicates it should be!");
			} else if (filterByAll.equalsIgnoreCase("f")) {
				if (adminSearchPage.isAllFilterChecked()) {
					adminSearchPage.clickAllCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isAllFilterChecked(),
						"All filter IS checked but data indicates it should NOT be!");
			}
		}
		...
		if (filterByUnmatched != null) {
			if (filterByUnmatched.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isUnmatchedChecked(),
						"Unmatched filter NOT checked but data indicates it should be!");
			} else if (filterByUnmatched.equalsIgnoreCase("f")) {
				if (adminSearchPage.isUnmatchedChecked()) {
					adminSearchPage.clickUnmatchedCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isUnmatchedChecked(),
						"Unmatched filtered IS checked but data indicates it should NOT be!");
			}
		}
		...
		if (filterByNotScheduled != null) {
			if (filterByNotScheduled.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isNotScheduledChecked(),
						"Not Scheduled filter NOT checked but data indicates it should be!");
			} else if (filterByNotScheduled.equalsIgnoreCase("f")) {
				if (adminSearchPage.isNotScheduledChecked()) {
					adminSearchPage.clickNotScheduledCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isNotScheduledChecked(),
						"Not Scheduled filtered IS checked but data indicates it should NOT be!");
			}
		}
		...
		if (filterByScheduled != null) {
			if (filterByScheduled.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isScheduledChecked(),
						"Scheduled filter NOT checked but data indicates it should be!");
			} else if (filterByScheduled.equalsIgnoreCase("f")) {
				if (adminSearchPage.isScheduledChecked()) {
					adminSearchPage.clickScheduledCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isScheduledChecked(),
						"Scheduled filtered IS checked but data indicates it should NOT be!");
			}
		}
		...
		if (filterByPaid != null) {
			if (filterByPaid.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isPaidChecked(),
						"Paid filter NOT checked but data indicates it should be!");
			} else if (filterByPaid.equalsIgnoreCase("f")) {
				if (adminSearchPage.isPaidChecked()) {
					adminSearchPage.clickPaidCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isPaidChecked(),
						"Paid filtered IS checked but data indicates it should NOT be!");
			}
		}
		...
		if (filterByError != null) {
			if (filterByError.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isErrorChecked(),
						"Error filter NOT checked but data indicates it should be!");
			} else if (filterByError.equalsIgnoreCase("f")) {
				if (adminSearchPage.isErrorChecked()) {
					adminSearchPage.clickErrorCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isErrorChecked(),
						"Error filtered IS checked but data indicates it should NOT be!");
			}
		}
		...
		if (filterByCanceled != null) {
			if (filterByCanceled.equalsIgnoreCase("t")) {
				...
				Assert.assertTrue(adminSearchPage.isCanceledChecked(),
						"Canceled filter NOT checked but data indicates it should be!");
			} else if (filterByCanceled.equalsIgnoreCase("f")) {
				if (adminSearchPage.isCanceledChecked()) {
					adminSearchPage.clickCanceledCheckBox();
				}

				Assert.assertFalse(adminSearchPage.isCanceledChecked(),
						"Canceled filtered IS checked but data indicates it should NOT be!");
			}
		}
		...
		WebElement loadingGif = driver
				.findElement(By.xpath(".//*[@id='angularScope']/div[1]/div/div[2]/div/div/div[2]/img"));
		...
		if (assertFilterByUnmatched)
			Assert.assertTrue(adminSearchPage.getTotalRecordCount() <= maxRecordCount,
					"Filter by UNMATCHED not found in the first row of data returned");

		if (assertFilterByNotScheduled)
			Assert.assertTrue(adminSearchPage.getTotalRecordCount() <= maxRecordCount,
					"Filter by NOT SCHEDULE not found in the first row of data returned");

		if (assertFilterByScheduled)
			Assert.assertTrue(adminSearchPage.getTotalRecordCount() <= maxRecordCount,
					"Filter by SCHEDULED not found in the first row of data returned");

		if (assertFilterByPaid)
			Assert.assertTrue(adminSearchPage.getTotalRecordCount() <= maxRecordCount,
					"Filter by PAID not found in the first row of data returned");

		if (assertFilteredByError)
			Assert.assertTrue(adminSearchPage.getTotalRecordCount() <= maxRecordCount,
					"Filter by ERROR not found in the first row of data returned");
